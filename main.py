...
def extractProducts(soup, brand):
  ...
  if itemsContainer:
    for item in itemsContainer.find_all('li', class_='promotion-item'):
      ...
      if brand == 'Todas' or (productBrand
                              and productBrand.lower() in brand.lower()):
        ...
        originalPrice = getPrice(
            item, 's',
            'andes-money-amount andes-money-amount-combo__previous-value andes-money-amount--previous andes-money-amount--cents-superscript'
        )
        ...
        discountPercent = ((originalPrice - discountPrice) / originalPrice *
                           100) if originalPrice and discountPrice else None
        ...
        products.append({
            ...
            round(discountPercent, 2) if discountPercent is not None else None,
            'Link':
            url,
        })
...
def startSearch(brand):
  ...
  products = []
  brandsCounter = Counter()  # Inicializa o contador fora do loop
  ...
  for page in range(1, 21):
    ...
    brandsCounter.update(brandsCounterPage)  # Atualiza o contador
